...
| F3b | 移动端离线草稿与断网补传 | 离线可用的条目/照片暂存、联网自动补传、幂等去重、多端时间戳合并与用户确认 |
...
### 5.3b F3b 移动端离线草稿与断网补传

移动端在现场记录时最容易遇到弱网与断网。本能力保证：断网可写、联网自动补、重试不产生重复数据、多端同时改时不互相覆盖。

#### 5.3b.1 入口

- 编辑页（`/spots/new`、`/spots/:uuid/edit`）在任何网络状态下都可填写、拍照、保存；
- 顶栏显示「离线 / 待补传 N / 待合并」状态徽标；
- 「我的记录」顶部集中展示尚未与服务端一致的本机草稿。

#### 5.3b.2 处理

1. **本地持久化。** 草稿文本与待补传照片（原始 `File/Blob`）写入 IndexedDB，不依赖内存；刷新、杀进程后仍在。
2. **自动补传。** 监听到 `online` 事件或页面重新可见时，自动按顺序补传：先补传照片，再保存条目，最后按用户原意提交审核。服务端错误用指数退避（30s 起，封顶 30 分钟），手动「立即补传」可跳过退避。
3. **幂等去重。** 创建草稿的请求带 `Idempotency-Key`（由本地草稿 ID 派生）。同用户同键 24 小时内重试直接返回首次结果；同键不同体返回 422 `IDEMPOTENCY_REPLAY_MISMATCH`。照片上传按内容哈希天然去重。
4. **字段级时间戳。** 每个字段（含 `attributes` 下的每个属性，键形如 `attributes.has_backrest`）各自记录最后修改时间（ISO 8601）。服务端在 `spot_field_timestamps` 表维护，作者可见的详情接口会回传。
5. **乐观并发检测。** 补传已存在条目时带 `baseUpdatedAt`（离线编辑所基于的版本）。服务端版本更新时返回 409 `SPOT_VERSION_CONFLICT`，响应体包含按时间戳预生成的合并方案（共同基线 / 服务端值 / 本机值 / 冲突字段 / 每字段预选较新一方），**不做整条后写覆盖**。
6. **用户确认后入库。** 全局弹出合并对话框：不冲突的字段自动合并；两端都改且不同的字段逐项列出双方内容与修改时间，默认选中时间较新的一方，用户可逐项改选。只有用户点「确认合并结果并保存」后才调用 `POST /spots/:uuid/merge` 入库。确认期间服务端又被修改时再次返回 409，用户须基于最新内容重新确认。
7. **属性按 key 合并。** 合并确认只覆盖用户确认过的属性键，同分类下未参与本次确认的属性保留服务端现值。

#### 5.3b.3 结果

- 离线保存的内容在本机可随时继续编辑；
- 补传成功后本地草稿与服务端版本一致，待补传照片自动清理；
- 多端分叉的内容在用户确认前不会静默覆盖任一方。

#### 5.3b.4 反馈

- 离线保存：「草稿已保存在本机，联网后自动补传」；
- 待补传：顶栏徽标 + 我的记录页状态标签（等待补传 / 正在补传照片 / 正在同步 / 补传失败原因 / 多端冲突待确认）；
- 冲突：全局合并对话框，逐字段展示双方内容与时间，确认后提示「合并结果已保存」。

#### 5.3b.5 闭环出口

- 离线暂存 → 联网自动补传 → 成功（或冲突 → 用户确认 → 成功）；
- 补传失败（服务端拒绝）→ 状态展示原因 → 用户修改后手动重试；
- 未补传的本地草稿可在「我的记录」里继续编辑或删除；
- 意图为「提交审核」的草稿，内容补传成功但提交动作失败时，下轮队列只补提交，不重复保存。
...
| POST | `/spots/:uuid/merge` | 作者 | 多端冲突经用户确认后的字段级合并入库（见 5.3b） |
...
- 写请求可带 `Idempotency-Key`（≤ 64 字符）头；同用户同键 24 小时内返回首次结果，键同体不同返回 422 `IDEMPOTENCY_REPLAY_MISMATCH`。
- `PATCH /spots/:uuid` 可带：
  - `baseUpdatedAt`：离线编辑所基于的条目版本；落后于服务端时返回 409 `SPOT_VERSION_CONFLICT`；
  - `fieldTimestamps`：逐字段（属性键形如 `attributes.has_backrest`）最后修改时间。
- 409 响应 `error.details` 形如：
  ```json
  {
    "spotUuid": "…",
    "serverUpdatedAt": "2026-09-19T12:00:00.000Z",
    "proposal": {
      "merged": { "title": "…", "attributes.count": 2 },
      "conflicts": [
        { "field": "title", "base": "…", "server": "…", "client": "…",
          "serverUpdatedAt": "…", "clientUpdatedAt": "…", "winner": "server" }
      ]
    }
  }
  ```
- `POST /spots/:uuid/merge` 请求：
  ```json
  {
    "fields": { "title": "最终标题", "attributes.count": 2 },
    "baseUpdatedAt": "2026-09-19T12:00:00.000Z",
    "fieldTimestamps": { "title": "2026-09-19T12:05:00.000Z" }
  }
  ```
  属性字段按 `attributes.<key>` 逐键合并，未出现的属性保留服务端现值；`baseUpdatedAt` 再次落后时仍返回 409。
- 作者视角的 `GET /spots/:uuid` 响应额外带 `fieldTimestamps`（公开响应不带）。

### 10.5 上传与隐私| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
...
- [ ] 断网状态下可填写、拍照、保存草稿，刷新与杀进程后草稿仍在
- [ ] 联网恢复后草稿与照片自动补传，顶栏/我的记录页可见补传状态
- [ ] 同一草稿断网重试多次只创建一条服务端记录（幂等键生效）
- [ ] 同幂等键不同请求体返回 422
- [ ] 两端各自修改不同字段时自动合并，无冲突弹窗
- [ ] 两端修改同一字段时弹出合并对话框，默认按时间戳选较新一方，可逐项改选
- [ ] 用户确认前服务端不被任一方覆盖；确认后按最终结果入库
- [ ] 合并确认单个属性不会冲掉同分类下的其他属性
- [ ] 确认期间服务端再次被修改时重新返回 409 要求再次确认
- [ ] 意图为"提交审核"的草稿补传成功后自动提交；提交失败不重复保存
